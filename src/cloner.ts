...
export async function cloneChannel(
  ...
  // メッセージをすべて舐める
  const LIMIT = 100;
  const options: FetchMessagesOptions = { limit: LIMIT, after: '0' };
  while (true) {
    ...
    for (const message of fetchedMessages.reverse().values()) {
      ...
      if (message.content === '' && message.embeds.length === 0 && message.attachments.size === 0 && message.components.length === 0) {
        continue;
      }

      // メンバーを取得
      const member = message.member ?? (await message.guild?.members.fetch(message.author.id).catch(() => undefined)) ?? message.author;
